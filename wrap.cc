...
public:
  InStream(GoInterface r) { this->r = r; }
  ...
  int Seek(__int64 offset, unsigned int seekOrigin,
           unsigned __int64 *newPosition) {
    return go_ReaderSeek(r, (GoInt64)offset, (GoInt)seekOrigin,
                         (GoInt64 *)newPosition);
  }
...
public:
  OutStream(GoInterface w) { this->w = w; }

  int GetSize() const { return go_WriterSize(w); }
  ...
  int Seek(__int64 offset, unsigned int seekOrigin,
           unsigned __int64 *newPosition) {
    return go_WriterSeek(w, (GoInt64)offset, (GoInt)seekOrigin,
                         (GoInt64 *)newPosition);
  }
...
bool lib7zip_open_archive(void *rp, c7z_Library *l, c7z_InStream **stream,
                          c7z_Archive **a) {
...
bool lib7zip_archive_extract_password(c7z_Archive *a, unsigned int i, void *wp,
                                      wchar_t *ws) {
  GoInterface w = *static_cast<GoInterface *>(wp);
  OutStream outstream(w);
  auto stream = static_cast<c7z_OutStream *>(&outstream);
...
wchar_t *lib7zip_wchar_array(wchar_t **wss, int i) { return wss[i]; }
